 ...
 */
function extractTokenFromResponse<T>(
  response: AxiosResponse<AuthResponse<T>>
): string | null {
...
export const authService = {
    ...
    try {
      ...
      if (authUrl) {
        ...
        if (authUrl.includes("/oauth/authorize")) {
          console.warn(
            "Fixing OAuth endpoint: /oauth/authorize -> /auth (from discovery document)"
          );
          ...
          console.warn(
            "Fixing OAuth endpoint: /oauth2/authorize -> /auth (from discovery document)"
          );
      ...
      console.log("=== getLoginUrl: No valid authorization URL found ===");

      console.error("=== getLoginUrl: Invalid response ===");
      console.error("Response data:", data);
      throw new Error(
        "Invalid authorization URL received from backend. Response: " +
          JSON.stringify(data)
      );
    } catch (error) {
  ...
  async getCurrentUser(): Promise<AuthResponse<UserData>> {
    const response = await axiosInstance.get<AuthResponse<UserData>>(
      AUTH_ENDPOINTS.getUserMe
    );
  ...
  submitLogoutForm(
    logoutUrl: string,
    params: { id_token_hint: string; post_logout_redirect_uri: string }
  ): void {
    console.log("Building POST form for logout...");
    console.log("Logout URL (action):", logoutUrl);
    console.log(
      "id_token_hint:",
      params.id_token_hint.substring(0, 20) + "..."
    );
    ...
    console.log(
      "  - id_token_hint:",
      idTokenInput.value.substring(0, 20) + "..."
    );
  ...
  async getLogoutUrl(): Promise<{ baseUrl: string; idTokenHint: string }> {
    ...
    const response = await axiosInstance.get<AuthResponse<string>>(
      AUTH_ENDPOINTS.getLogout
    );
    const data = await handleAxiosResponse(response);
  ...
  async logout(): Promise<AuthResponse<void>> {
    ...
    try {
      const response = await axiosInstance.post<AuthResponse<void>>(
